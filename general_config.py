from modules.blocks import BaseConfig
from modules.unet import ScaleAt

# from model.latentnet import *
from modules.latentnet import dataclass, LatentNetType, MLPSkipNetConfig
...
# from dataset import *
from dataset import FFHQlmdb, Horse_lmdb, CelebAlmdb, use_cached_dataset_path
...
# from model import *
from modules import BeatGANsAutoencConfig, BeatGANsUNetConfig, ModelConfig
...
# from dataset_util import distributed
...
class TrainConfig(BaseConfig):
    ...
    eval_path: str = None
    base_dir: str = "src/DiffAE/checkpoints"
...
#     conf.latent_infer_path = f"src/DiffAE/checkpoints/{ffhq128_autoenc_130M().name}/latent.pkl"
#     conf.batch_size = 32
#     conf.lr = 1e-3
...
#         "130M",
#         f"src/DiffAE/checkpoints/{ffhq128_autoenc_130M().name}/last.ckpt",
#     )
...
def ffhq256_autoenc_cls():
    ...
    conf.manipulate_mode = ManipulateMode.celebahq_all
    conf.manipulate_znormalize = True
    conf.latent_infer_path = f"src/DiffAE/checkpoints/{ffhq256_autoenc().name}/latent.pkl"  # we train on Celeb dataset, not FFHQ
    conf.batch_size = 32
    conf.lr = 1e-3
    conf.total_samples = 300_000
    # use the pretraining trick instead of contiuning trick
    conf.pretrain = PretrainConfig(
        "130M",
        f"src/DiffAE/checkpoints/{ffhq256_autoenc().name}/last.ckpt",
    )
